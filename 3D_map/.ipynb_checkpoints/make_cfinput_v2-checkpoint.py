...
# Usage example: python make_cfinput_v2.py --map Results/cumulative_reduced3Dmap-8_set_1.npz --distance_step 1 --verbose --cmp YlOrBr --npoints 10000 --plot3D --plot2D --plotsphere --save
...
def read_data():
	...
	distance_step = args.distance_step
	distance_bins = np.arange(0,distance_bin_number,distance_step)
	distance_max = distance_bins[-1]*distance_step
...
def plot3Dmap(ras, decs, zs, weights):
	...
	scatter = ax3D.scatter(ras, np.cos(decs), zs, c=weights, marker=',', cmap=cm, alpha=alpha, zorder=-100*np.mean(zs))#, depthshade=False)
...
#	ax3D.set_zlim((-plot_axis_max,plot_axis_max))
	...
	plt.savefig('cumulative_3Dmap_rdz-'+str(nside)+'-'+str(args.npoints)+'_set_'+set_name +'.png')
	plt.close()
...
def plot2Dmap(ras, decs, zs, weights):
	...
	plt.savefig('cumulative_2Dmap_rdz-'+str(nside)+'-'+str(args.npoints)+'_set_'+set_name +'.png')
	plt.close()
...
def plotspheremap(ras, decs, zs, weights):
	...
	scatter = axsphere.scatter(x,y,z, c=color, s=size, marker=',', cmap=cm, alpha=alpha,zorder=-100*np.mean(zs))#,zorder=-100*zs)
	...
	plt.savefig('cumulative_spheremap_rdz-'+str(nside)+'-'+str(args.npoints)+'_set_'+set_name +'.png')
	plt.close()
...
if __name__ == "__main__":
	...
	if args.verbose:
		print('Selecting the ' + str(args.npoints) + ' random pixels based on the probability distribution ('+str(np.round(args.npoints/dim*100,1))+'% of the total).')
	...
	if args.save:
		...
		save_name = args.map.split('.')[0]
		save_name = save_name.split('/')[1]
		print (save_name)        
		set_name = save_name.split('_')[3]
		np.savetxt('cf_input_'+save_name+'-'+str(args.npoints)+'.txt', matrix_output, fmt='%.6f,%.6f,%.6f,%.8e', delimiter=',', newline='\n', header=header, footer='', comments='# ', encoding=None)

	#print(ras);print(decs);print(points);print(distances);print(zs);print(weights)	#FOR TESTING

	if args.plot3D:
		plot3Dmap(ras, decs, zs, weights)
	
	if args.plot2D:
		plot2Dmap(ras, decs, zs, weights)
