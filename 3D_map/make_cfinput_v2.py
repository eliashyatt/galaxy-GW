...
# Usage example: ./make_cfinput_v1.py --map Results/cumulative_reduced3Dmap-8_set_1.npz --distance_step 1 --verbose --cmp YlOrBr --npoints 10000 --plot3D --plot2D --plotsphere --save
...
def plot2Dmap(ras, decs, zs, weights):
	...
	size = 10**(2*(zs-np.max(zs))/(np.min(zs)-np.max(zs)))
