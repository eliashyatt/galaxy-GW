...
# Usage example: python make_cumulative3D_v1.py --verbose --map /home/vboxuser/Desktop/Code_for_Thesis/galaxy-GW/GW_bayestar/v2/skymaps/the_test_ten/flat_fits --save_name set_1\
#					--nside_out 256 --max_distance 1400 --num_distance_bins 8 \
...
def makes_everything(filename):
	*_, root_filename = filename.split('/')
	root_filename, *_ = root_filename.split('.')
	if verbose:
...
if __name__ == "__main__":
	...
	parser.add_argument('--map', help='Directory with maps or file with list of maps in fits format', required=True)
	...
	parser.add_argument('--save_name', help='Save name for the cumulative map file')
	parser.add_argument('--save_path', help='Save path for the cumulative map file', default='Results',type=str)
	args = parser.parse_args()
	print ("Read all the variables")    
	...
	try:
		#map_list = [line.rstrip() for line in open(filename)]
		map_list = glob.glob(filename+'/*.fits')
		if verbose:
			print('Working with list of events in',filename)
	except:
		map_list = [filename]
	...
	for mapfile in map_list:
		try:
			computed_maps+=1
			if verbose:
				print('\n Working with map: '+str(computed_maps)+'/'+str(len(map_list)))
...
# create the save directory        
	...
	if not check_folder:
		...
		pass
# get the save name    
#	s*_, save_name = filename.split('n34/')
#	save_name, *_ = save_name.split('/') 
    
	np.savez_compressed(save_path + '/cumulative_reduced3Dmap-'+str(nside_low)+'_'+ str(args.save_name) +'.npz', cumulative_reduced3Dmap=sum_reduced3D)
