...
#  --sampling SAMPLING  Point sampling scheme (dist, max, random, distance_dist, distance_max, div_dist, or div_max). Default: dist.
...
#######
	...
	if sampling == 1:  #dist
		...
		#nz_idx = flattened_data != 0
		nz_idx = np.where(flattened_data != 0)[0]
		flattened_data = flattened_data[nz_idx]
		distances = distances[nz_idx]
	...
	elif sampling == 3 or sampling == 4:   #distance

		n_total = npoints * n_dist
		selected_pix = np.empty(n_total, dtype=np.int64)
		selected_distances = np.empty(n_total, dtype=np.int64)
		selected_weights = np.empty(n_total, dtype=np.float64)

		idx = 0
		bin_probs = np.zeros(n_dist)

		for d in range(n_dist):
			bin_data = data[:, d+1] 
			bin_prob = np.sum(bin_data)
			bin_probs[d] = bin_prob
		...
		bin_choices = np.searchsorted(cdf, r)

		for d in range(n_dist):
			bin_points = len(np.where(bin_choices == d)[0])
			if bin_points == 0:
				continue

			bin_data = data[:, d + 1]
			total = np.sum(bin_data)
			if total <= 0:
				continue
		
			probabilities = bin_data / total

			if sampling == 3:
				cdf = np.cumsum(probabilities)
				cdf /= cdf[-1]

				r = np.random.random(bin_points)
				r = np.minimum(r, 1.0 - 1e-15)

				selected_points = np.searchsorted(cdf, r)
			
			else:
				selected_points = np.argpartition(-bin_data, bin_points - 1)[:bin_points]

			selected_pix[idx:idx+len(selected_points)] = selected_points
			selected_distances[idx:idx+len(selected_points)] = d * distance_step
			selected_weights[idx:idx+len(selected_points)] = bin_data[selected_points]
	...
	elif sampling == 5 or sampling == 6:    #div_dist or div_max

		n_total = npoints
		n_per_bin = npoints / (n_dist - 1)
		#print("Points:", npoints)
		#print("Distance Bins:", n_dist)
		print("Points per bin:", n_per_bin)
		...
		for d in range(n_dist):
			...
			if sampling == 5:
				...
				r = np.random.random(n_per_bin)
				r = np.minimum(r, 1.0 - 1e-15)

				selected_points = np.searchsorted(cdf, r)

			else:
				selected_points = np.argpartition(-bin_data, n_per_bin - 1)[:n_per_bin]
...
if __name__ == "__main__":
	...
	parser.add_argument('--sampling', help='Point sampling scheme (dist, max, random, distance_dist, distance_max, div_dist, or div_max). Default: dist.', required=False, default='dist')
	args = parser.parse_args()
	
	check_filename = 'cf_input_' + args.map[:-4] + '-' + str(args.npoints) + '_' + args.sampling + '.txt'
	...
	if os.path.exists(check_filename):
		...
		if args.sampling == 'max':
			sampling = 0 
		elif args.sampling == 'dist':
			sampling = 1
		elif args.sampling == 'random':
			sampling = 2
		elif args.sampling == 'distance_dist':
			sampling = 3
		elif args.sampling == 'distance_max':
			sampling = 4
		elif args.sampling == 'div_dist':
			sampling = 5
		elif args.sampling == 'div_max':
			sampling = 6
		else:
			print('I do not understand what sampling you want. Try again. I am quitting now!')
			sys.exit()
		...
		if args.save:
			...
			#if sampling == 1:
			header = 'Sample of ' + str(args.npoints) + ' probability pixels from ' + args.map + ' using ' + args.sampling + ' method' + '\n' + 'dec,ra,d_l,weight'
			#else:
			...
			np.savetxt('cf_input_'+args.map.split('.')[0]+'-'+str(args.npoints)+'_'+str(args.sampling)+'.txt', matrix_output, fmt='%.6f,%.6f,%.6f,%.8e', delimiter=',', newline='\n', header=header, footer='', comments='# ', encoding=None)
