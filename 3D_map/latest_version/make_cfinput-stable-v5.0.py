...
#  --sampling SAMPLING  Point sampling scheme (dist, max, random, distance, div_dist, or div_max). Default: dist.
...
def read_data():
	...
	distance_step = int(parameters[3][:-14])
	...
	if sampling == 1:  #dist
		...
		for d in range(n_dist):
			...
			if sampling == 4:
				...
				selected_points = np.argpartition(-bin_data, npoints - 1)[:npoints]
...
if __name__ == "__main__":
	...
	parser.add_argument('--sampling', help='Point sampling scheme (dist, max, random, distance, div_dist, or div_max). Default: dist.', required=False, default='dist')
