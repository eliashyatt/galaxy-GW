...
GW_df_full['luminosity_distance'] = GW_df_full[distance_col]
...
def norm(dd, rr, gdr, wdr, z_bin, d_L_bin):
    dd_norm = dd['npairs'] / (len(z_bin) * len(d_L_bin))
    rr_norm = rr['npairs'] / (r2dr * len(z_bin) * r2dr * len(d_L_bin))
    gdr_norm = gdr['npairs'] / (len(z_bin) * r2dr * len(d_L_bin))
    wdr_norm = wdr['npairs'] / (r2dr * len(z_bin) * len(d_L_bin))
...
def w_no_jk(z_bin, d_L_bin, all_data):
    ...
    dd_norm, rr_norm, gdr_norm, wdr_norm = norm(dd, rr, gdr, wdr, z_bin, d_L_bin)
...
def jk_loop(all_data, all_regions, i, w0):
    ...
    if i in galaxy_regions or i in GW_regions:
        ...
        dd_norm, rr_norm, gdr_norm, wdr_norm = norm(dd, rr, gdr, wdr, galaxy_sample[0], GW_sample[0])
        w = w_func(dd_norm, rr_norm, gdr_norm, wdr_norm)
    else:
        w = w0
    return w
