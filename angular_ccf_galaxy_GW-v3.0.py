...
if deg_or_rad == 'rad':
    ...
    if np.max(GW_df_full['dec']) > (np.pi / 2):
        ...
        GW_df_full['Dec_deg'] = GW_df_full['dec'] * 180 / np.pi
    ...
    #else:
        #galaxy_df_full['Dec_deg'] = galaxy_df_full['dec'] * 180 / np.pi


#accounts for no weight column
if 'weight' not in GW_df_full.columns:
    print('No weight column in GW dataset. Assigning weight = 1.0 to all events.')
...
def random_weights(GW_weights, d_L_bin):
    ...
    GW_random_weights = np.random.choice(GW_weights, size=len(d_L_bin)*r2dr, replace=True)
    #print(np.min(GW_random_weights))
...
#normalizes dd and rr based on the weights
...
def w_no_jk(z_bin, d_L_bin, all_data):
    ...
    #dd = dd_rr_func(theta_bin, galaxy_random, GW_random, GW_random_weights)
    #rr = dd_rr_func(theta_bin, galaxy_random, GW_random, GW_random_weights)
    #gdr = dd_rr_func(theta_bin, galaxy_random, GW_random, GW_random_weights)
    #wdr = dd_rr_func(theta_bin, galaxy_random, GW_random, GW_random_weights)
...
def jk_loop(all_data, all_regions, i, w0):
    ...
    if i in galaxy_regions or i in GW_regions:
        ...
        #print("Ng =", len(galaxy[0]))
        #print("NGW =", len(GW[0]))
        #print("NRg =", len(galaxy_random[0]))
        #print("NRGW =", len(GW_random[0]))
...
def run_code(z_iter, d_L_iter):
    ...
    #print(galaxy_ra.min(), galaxy_ra.max())
    #print(galaxy_dec.min(), galaxy_dec.max())
    #print(galaxy_random[0].min(), galaxy_random[0].max())
    #print(galaxy_random[1].min(), galaxy_random[1].max())

    #print(GW_ra.min(), GW_ra.max())
    #print(GW_dec.min(), GW_dec.max())
    #print(GW_random[0].min(), GW_random[0].max())
    #print(GW_random[1].min(), GW_random[1].max())
...
for i in range(z_bin_num):
    num_in_bin = len(redshift_bin(i))
    print("Number of galaxies in z bin", i+1, ":", num_in_bin)

for i in range(d_L_bin_num):
    num_in_bin = len(luminosity_distance_bin(i))
    print("Number of GW sources in d_L bin", i+1, ":", num_in_bin)

GW_a, GW_loc, GW_scale = 1.9094, 0, 1319.36
plt.hist(GW_df['luminosity_distance'], density=True)
plt.plot(np.linspace(GW_df['luminosity_distance'].min(), GW_df['luminosity_distance'].max()), stats.gamma.pdf(np.linspace(GW_df['luminosity_distance'].min(), GW_df['luminosity_distance'].max()), GW_a, loc=GW_loc, scale=GW_scale), 'r-', lw=2)
plt.xlabel("Luminosity Distance")
plt.ylabel("Density")
plt.title("GW Luminosity Distance Distribution")
plt.show()
...
theta_bin = np.array([0.00000001, theta_max_deg])
